// ===== Trade Pulse AI – community.js (FULL with withdrawal proof messages, timestamp fix) =====
(function() {
  console.log('community.js loaded with organic chat refinements');
  ...
  // ========== SINGLE SHUFFLE FUNCTION (used everywhere) ==========
  ...
  function getWithdrawalProofMessage(persona) {
    const countryBanks = {
      Nigeria: [
        ...
        "Withdrawal hit my Opay like play. Proof dey!",
        "Abeg make una see my withdrawal. E no take time.",
        "Omo, I don receive alert for my withdrawal. See proof.",
        "Withdrawal don enter! Make I show una the screenshot.",
        "Na who get money dey sleep? Withdrawal just land.",
        "My people, see as money enter my account sharp sharp.",
        "I no fit shout, withdrawal dey work perfectly."
      ],
      Mexico: [
        "Acabo de retirar sin problemas. Aquí la prueba.",
        "Ya me llegó el retiro, rapidísimo. ¡Miren!",
        "Otro retiro exitoso. La prueba habla por sí sola.",
        "Pura ganancia, compa. Aquí está el comprobante.",
        "Retiro confirmado. Plata en la cuenta. ¡Sí se puede!",
        "Ni un minuto tardó. Miren la captura.",
        "Retiré $200 dólares en un santiamén. ¡Vean la prueba!",
        "La plata ya está en mi banco. Todo legal."
      ],
      default: [
        "Just withdrew successfully. Proof attached!",
        "Money arrived in 5 minutes. Here's the screenshot.",
        "Another smooth withdrawal. This platform is legit!",
        "Cashed out today. Proof below.",
        "Withdrawal completed. No stress at all.",
        "Fast withdrawal as always. Screenshot for proof.",
        "Received my money. Thank you Trade Pulse AI!",
        "Proof of withdrawal posted. Don't miss out!"
      ]
    };
    ...
    for (let attempt = 0; attempt < 50; attempt++) {
      ...
      if (!recentProofMessages.has(candidate)) {
        msg = candidate;
        break;
      }
    }
    if (!msg) {
      recentProofMessages.clear();
      msg = bank[Math.floor(Math.random() * bank.length)];
    }

    recentProofMessages.add(msg);
    if (recentProofMessages.size > MAX_RECENT_PROOF_MSGS) {
      const iter = recentProofMessages.values();
      recentProofMessages.delete(iter.next().value);
    }
  ...
  function createProofChatCard(proofPersona) {
    ...
    return `
      <div class="community-chat-card" data-msg-timestamp="${Date.now()}">
        <div class="chat-msg-top">
          ${avatarHtml}
          <div>
            <div class="chat-msg-name">${persona.name}</div>
            <div style="font-size:11px;color:#6b7280;">Just now</div>
          </div>
        </div>
        <div class="chat-msg-body">${message}</div>
        <div class="chat-proof-card">
          <img src="${proofPersona.proofImage}" style="width:100%; border-radius:8px;" onerror="this.style.display='none'; this.nextElementSibling.style.display='block';">
          <div style="display:none; padding:12px; text-align:center; color:#9ca3af; font-size:13px;">📎 Proof image</div>
        </div>
  ...
  let shuffledProofPersonas = [];
  let proofIndex = 0;
  let lastProofTimestamp = 0;

  function resetProofCycle() {
    shuffledProofPersonas = shuffleArray([...withdrawalProofPersonas]);
    proofIndex = 0;
  }

  function getNextProofPersona() {
    if (proofIndex >= shuffledProofPersonas.length) {
      resetProofCycle();
    }
    return shuffledProofPersonas[proofIndex++];
  }
  ...
  // ========== CHAT PACE: add messages gradually ==========
  ...
  // ========== ANNOUNCEMENTS WITH USER-SPECIFIC DATES (FIXED) ==========
  ...
  document.querySelector('.community-invite-btn')?.addEventListener('click', () => {
    ...
    const inviteText = `🚀 Join me on Trade Pulse AI and start earning daily profits!%0AUse my referral code *${code}* when you sign up and both of us get a bonus.%0A👉 Sign up here: https://tradepulseaibot.github.io/Trade-Pulse-AI/?ref=${code}`;
  ...
  function startCommunitySimulation() {
    try {
      ...
      // ---- WITHDRAWAL PROOF MESSAGES ----
      resetProofCycle();

      setInterval(() => {
        if (!communityView.classList.contains('active')) return;
        const now = Date.now();
        if (now - lastProofTimestamp < 45000) return;
        if (Math.random() > 0.8) return; // testing – change back to 0.4 later
