// ===== Trade Pulse AI – main.js (FULL with admin toggle + forced demo, chart seed, withdraw navigation unrestricted) =====
(function() {
  ...
  function isForcedDemoActive() {
    return localStorage.getItem('adminDemoMode') === 'true' && !isAdminLoggedIn();
  }

  // Shared KYC state for forced demo (stored in localStorage)
  function getForcedDemoKyc() {
    try { return JSON.parse(localStorage.getItem('forcedDemoKyc') || 'null'); } catch { return null; }
  }
  function setForcedDemoKyc(data) {
    localStorage.setItem('forcedDemoKyc', JSON.stringify(data));
  }
  function clearForcedDemoKyc() {
    localStorage.removeItem('forcedDemoKyc');
  }
  ...
  function saveUser(user) {
    ...
    updateAll();
    if (isAdminLoggedIn()) addAdminToggleUI();
  }
  ...
  function updateDrawerUserInfo() {
    ...
    if (kycBadge) {
      if (isForcedDemoActive()) {
        const forcedKyc = getForcedDemoKyc();
  ...
  // ========== BANK ICON / TX DISPLAY (unchanged) ==========
  const bankIconMap = {
  ...
  function updateAll() {
    // Force demo mode override (balance only initialised once, no saveUser)
    if (isForcedDemoActive()) {
      const user = getStoredUser();
      if (user) {
        user.kycStatus = getForcedDemoKyc()?.status || 'unverified';
        user.kycVerified = false;
      }
      if (base.homeBalance === 0) {
        base.homeBalance = 10000;
        base.walletTotal = 10000;
        base.walletAvailable = 10000;
      }
      ...
      base.walletLocked = 0;
      base.withdrawable = 0;
      base.totalProfit = base.homeBalance - 10000;

      // Seed chart data so it shows immediately
      if (chartData.length === 0) {
        chartData.push({
          label: new Date().getHours() + ':' + String(new Date().getMinutes()).padStart(2, '0'),
          value: base.homeBalance
        });
      }
      // no saveUser – avoids lag
    }
    ...
    if (withdrawBtn) {
      if (isForcedDemoActive() || base.homeBalance <= 0) {
  ...
  function bindWithdrawFormEvents() {
    ...
    if (requestBtn) {
      requestBtn.addEventListener('click', () => {
        if (blockIfForcedDemo()) return; // forced demo guard – only on final button
  ...
  function openDepositModal() {
    // NO forced demo guard here – user can open the modal
  ...
  $('confirmDepositBtn')?.addEventListener('click', () => {
    if (blockIfForcedDemo()) return; // guard here
  ...
  // Bottom nav – withdraw page allowed (no guard)
  ...
  // Drawer / data-nav clicks – withdraw page allowed (no guard)
  ...
  // Referral modal fix: update data on open
  ...
  // Referral modal "How does it work?" toggle
  ...
  // Open referral modal with data refresh
  ...
  // ========== FORCED DEMO MODALS ==========
  function showKycUnverifiedModal() {
    const existing = document.getElementById('kycUnverifiedModal');
    if (existing) existing.remove();
    const modal = document.createElement('div');
    modal.id = 'kycUnverifiedModal';
    modal.className = 'modal-overlay open';
    modal.innerHTML = `
      <div class="modal-panel" style="max-width:400px;">
        <div class="modal-header">
          <div class="modal-title">KYC Unverified</div>
          <button class="modal-close-btn" id="closeKycUnverifiedModal">✕</button>
        </div>
        <div style="text-align:center; padding:20px 0;">
          <div style="font-size:48px;">⚠️</div>
          <div style="font-size:16px; font-weight:600; margin-bottom:8px;">Identity Verification Required</div>
          <div style="font-size:14px; color:#9ca3af;">Please complete KYC to access all features.</div>
          <button id="startKycFromModal" class="withdraw-btn" style="margin-top:20px;">Start Verification</button>
        </div>
      </div>
    `;
    document.body.appendChild(modal);
    document.getElementById('closeKycUnverifiedModal').onclick = () => modal.remove();
    document.getElementById('startKycFromModal').onclick = () => {
      modal.remove();
      if (typeof window.openKycModal === 'function') window.openKycModal();
    };
    modal.addEventListener('click', e => { if (e.target === modal) modal.remove(); });
  }

  function showKycProcessingModal() {
    const existing = document.getElementById('kycProcessingModal');
    if (existing) existing.remove();
    const modal = document.createElement('div');
    modal.id = 'kycProcessingModal';
    modal.className = 'modal-overlay open';
    modal.innerHTML = `
      <div class="modal-panel" style="max-width:400px;">
        <div class="modal-header">
          <div class="modal-title">KYC Under Review</div>
          <button class="modal-close-btn" id="closeKycProcessingModal">✕</button>
        </div>
        <div style="text-align:center; padding:20px 0;">
          <div class="spinner" style="width:44px; height:44px; border-width:3px; margin:0 auto 12px;"></div>
          <div style="font-size:16px; font-weight:600;">Verification in progress</div>
          <div style="font-size:14px; color:#9ca3af; margin-top:8px;">This usually takes 1–3 business days. You will be notified once complete.</div>
          <button id="contactSupportFromProcessing" class="withdraw-btn" style="margin-top:20px;">Contact Support</button>
        </div>
      </div>
    `;
    document.body.appendChild(modal);
    document.getElementById('closeKycProcessingModal').onclick = () => modal.remove();
    document.getElementById('contactSupportFromProcessing').onclick = () => window.open('https://t.me/trade_pulse_ai_support', '_blank');
    modal.addEventListener('click', e => { if (e.target === modal) modal.remove(); });
  }

  function showKycRejectedDetailModal() {
    const existing = document.getElementById('kycRejectedDetailModal');
    if (existing) existing.remove();
    const forcedKyc = getForcedDemoKyc();
    const frontSrc = forcedKyc?.images?.front || 'assets/kyc/example_front.jpg';
    const backSrc = forcedKyc?.images?.back || 'assets/kyc/example_back.jpg';

    const modal = document.createElement('div');
    modal.id = 'kycRejectedDetailModal';
    modal.className = 'modal-overlay open';
    modal.innerHTML = `
      <div class="modal-panel" style="max-width:400px;">
        <div class="modal-header">
          <div class="modal-title">KYC Rejected</div>
          <button class="modal-close-btn" id="closeKycRejectedDetailModal">✕</button>
        </div>
        <div style="text-align:center; margin-bottom:16px;">
          <div style="font-size:48px;">🚫</div>
          <div style="font-size:16px; font-weight:600; margin-bottom:8px;">Verification Failed</div>
          <div style="font-size:14px; color:#9ca3af; line-height:1.5;">
            Your submitted ID could not be verified. The images may be unclear or details do not match our records. Please contact support for further assistance.
          </div>
        </div>
        <div style="display:flex; gap:10px; justify-content:center; margin-bottom:16px;">
          <div style="text-align:center; flex:1; position:relative;">
            <div style="font-size:11px; color:#9ca3af; margin-bottom:4px;">Front</div>
            <div style="position:relative; border-radius:8px; overflow:hidden;">
              <img src="${frontSrc}" style="width:100%; max-height:120px; object-fit:contain; border-radius:8px; background:#1a1a2e;">
              <div style="position:absolute; top:0; left:0; right:0; bottom:0; background:rgba(0,0,0,0.55); display:flex; align-items:center; justify-content:center;">
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="3"><circle cx="12" cy="12" r="10"/><path d="M15 9l-6 6M9 9l6 6"/></svg>
              </div>
            </div>
          </div>
          <div style="text-align:center; flex:1; position:relative;">
            <div style="font-size:11px; color:#9ca3af; margin-bottom:4px;">Back</div>
            <div style="position:relative; border-radius:8px; overflow:hidden;">
              <img src="${backSrc}" style="width:100%; max-height:120px; object-fit:contain; border-radius:8px; background:#1a1a2e;">
              <div style="position:absolute; top:0; left:0; right:0; bottom:0; background:rgba(0,0,0,0.55); display:flex; align-items:center; justify-content:center;">
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="3"><circle cx="12" cy="12" r="10"/><path d="M15 9l-6 6M9 9l6 6"/></svg>
              </div>
            </div>
        ...
        <div id="retakeMessage" style="color:#f87171; font-size:13px; text-align:center; margin-bottom:12px; display:none;">
          Resubmission is currently locked. Please contact support.
        </div>
        <div style="display:flex; gap:10px;">
          <button id="retakeKycBtn" class="btn btn-outline" style="flex:1;">Retake Images</button>
          <button id="contactSupportFromRejectBtn" class="withdraw-btn" style="flex:1;">Contact Support</button>
        </div>
      </div>
    `;
    document.body.appendChild(modal);
    document.getElementById('closeKycRejectedDetailModal').onclick = () => modal.remove();
    modal.addEventListener('click', e => { if (e.target === modal) modal.remove(); });
    document.getElementById('retakeKycBtn').onclick = () => {
      document.getElementById('retakeMessage').style.display = 'block';
    };
    document.getElementById('contactSupportFromRejectBtn').onclick = () => window.open('https://t.me/trade_pulse_ai_support', '_blank');
  }
  window.showKycRejectedDetailModal = showKycRejectedDetailModal;

  function blockIfForcedDemo() {
    if (!isForcedDemoActive()) return false;
    const forcedKyc = getForcedDemoKyc();
    const status = forcedKyc?.status || 'unverified';
    if (status === 'processing') {
      showKycProcessingModal();
    } else if (status === 'rejected') {
      showKycRejectedDetailModal();
    } else {
      showKycUnverifiedModal();
    }
    return true;
  }

  // ========== ADMIN TOGGLE UI IN DRAWER ==========
  function addAdminToggleUI() {
    const drawerPanel = $('drawerPanel');
    if (!drawerPanel) return;
    const old = document.getElementById('adminToggleRow');
    if (old) old.remove();
    const row = document.createElement('div');
    row.id = 'adminToggleRow';
    row.className = 'drawer-menu-item';
    ...
    row.innerHTML = `
      <div class="drawer-menu-left">
        ...
        <span class="drawer-menu-label">Force Demo Mode</span>
      </div>
      <label style="position:relative; display:inline-block; width:44px; height:24px;">
        <input type="checkbox" id="adminDemoToggle" ${localStorage.getItem('adminDemoMode') === 'true' ? 'checked' : ''}>
        <span style="position:absolute; cursor:pointer; top:0; left:0; right:0; bottom:0; background:${localStorage.getItem('adminDemoMode') === 'true' ? '#a855f7' : 'rgba(255,255,255,.2)'}; border-radius:24px; transition:.3s;"></span>
      </label>
    `;
    const vipCard = drawerPanel.querySelector('.drawer-vip-card');
    if (vipCard) vipCard.insertAdjacentElement('afterend', row);
    else drawerPanel.appendChild(row);

    document.getElementById('adminDemoToggle').addEventListener('change', function() {
      const on = this.checked;
      localStorage.setItem('adminDemoMode', on ? 'true' : 'false');
      if (on) {
        if (!getForcedDemoKyc()) setForcedDemoKyc({ status: 'unverified' });
      } else {
        clearForcedDemoKyc();
      }
      updateAll();
  ...
  // ========== 2-day auto-reject timer ==========
  setInterval(() => {
    if (!isForcedDemoActive()) return;
    const forcedKyc = getForcedDemoKyc();
    if (forcedKyc?.status === 'processing' && forcedKyc.processingStart) {
      if (Date.now() - forcedKyc.processingStart >= 2 * 24 * 60 * 60 * 1000) {
        setForcedDemoKyc({ ...forcedKyc, status: 'rejected' });
        updateAll();
        updateDrawerUserInfo();
      }
    }
  ...
  // ========== CHART AUTO-GROWTH (FORCED DEMO MODE) ==========
  setInterval(() => {
    if (!$('homeView')?.classList.contains('active')) return;

    if (isForcedDemoActive()) {
      const dailyProfitRate = 0.03; // 3% per day
      const secondsInDay = 24 * 60 * 60;
      const growthPerSecond = 1 + (dailyProfitRate / secondsInDay);
      const growthPer30s = Math.pow(growthPerSecond, 30);

      const newBalance = base.homeBalance * growthPer30s;
      base.homeBalance = newBalance;
      base.walletTotal = newBalance;
      base.walletAvailable = newBalance;
      base.withdrawable = 0;
      base.totalProfit = newBalance - 10000;
